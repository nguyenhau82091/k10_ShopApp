...
class CartService {
  ...
  static Future<List<Cart>> removeCartItem(List<String> productId) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    List<Cart> existingCart = [];
    ...
    try {
      ...
      // Remove items with the specified productId from the cart
      existingCart
          .removeWhere((cartItem) => productId.contains(cartItem.idProduct));

      // Convert the updated cart to JSON strings and save
      final updatedCartStrings = existingCart
          .map((cartItem) => jsonEncode(cartItem.toJson()))
          .toList();
      await prefs.setStringList("cart", updatedCartStrings);

      print("Remove cart item thành công");

      return existingCart;
    } catch (e) {
      print("Lỗi khi xóa sản phẩm từ giỏ hàng: $e");
      return existingCart;
    }
  }
}
